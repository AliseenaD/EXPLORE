...
def predict_frames_multi(project_path, model, videos, time, ref_point, names, objects, bins, o_coord):
    ...
    def frame_prediction(video, time, vid_name, ref_point):
        ...
        csv_filename = os.path.join(os.path.dirname(video), f"{vid_name}_predictions.csv")
        with open(csv_filename, mode='w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
            
            # Write header
            header = ['Frame', 'Predicted_Class'] + [f'{name}_Probability' for name in names]
            ...
            for i in range(len(result)):
                predicted_class = names[int(result[i])]
                probabilities = result1[i]

                # Write prediction and probabilities to CSV
                row = [i+1, predicted_class] + list(probabilities)
        ...
        print(f'Frame-by-frame predictions saved to: {csv_filename}')
